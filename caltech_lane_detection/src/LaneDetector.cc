...
//     ipmHeight = 3*(ipmInfo->yLimits[1]-ipmInfo->yLimits[0]) / (lanesConf->lineHeight/3.);
//     ipmWidth = ipmHeight * 4/3;
//     //put into the conf
//     lanesConf->ipmWidth = int(ipmWidth);
//     lanesConf->ipmHeight = int(ipmHeight);

//     #ifdef DEBUG_GET_STOP_LINES
//     cout << "IPM width:" << lanesConf->ipmWidth << " IPM height:"
// 	 << lanesConf->ipmHeight << "\n";
//     #endif
...
#warning "Check: Smoothing IPM image"
  ...
  FLOAT stopLinePixelWidth = lanesConf->lineWidth *
      ipmInfo->xScale;
  //stop line pixel height: 12 inches = 12*25.4 mm
  FLOAT stopLinePixelHeight = lanesConf->lineHeight  *
      ipmInfo->yScale;
  //kernel dimensions
  //unsigned char wx = 2;
  //unsigned char wy = 2;
...
//     mcvFilterLines(ipmt, ipmt, lanesConf->kernelWidth,
...
//     cvReleaseMat(&ipm2);
  ...
  // 	FLOAT qtileThreshold = mcvGetQuantile(ipm, lanesConf->lowerQuantile);
  // 	mcvThresholdLower(ipm, ipm, qtileThreshold);

  //     qtileThreshold = mcvGetQuantile(ipm2, lanesConf->lowerQuantile);
...
// 		    state);
  ...
  mcvPostprocessLines(image, clrImage, fipm, ipm, *lanes, *lineScores,
                      *splines, *splines_world, *splineScores,
 ...
 * \param splines_world a vector of returned splines in world coordinates
...
// 	}
    ...
    splines_world = splines;
    //convert splines to world coordinates
    mcvSplinesImIPM2World(splines_world, ipmInfo);
...
/** This function converts splines from IPM image coordinates back to world
 * coordinates
 *
 * \param splines the input splines
 * \param ipmInfo the IPM info
 ...
 *
 */
void mcvSplinesImIPM2World(vector<Spline> &splines, IPMInfo &ipmInfo)
{
  //loop on splines and convert
  for (int i=0; i<(int)splines.size(); i++)
  {
    for(int j = 0; j < 4; j++) {
      mcvPointImIPM2World(&splines[i].points[j], &ipmInfo);
    }
    //get points for this spline in IPM image
    //CvMat *points = mcvEvalBezierSpline(splines[i], .1);
