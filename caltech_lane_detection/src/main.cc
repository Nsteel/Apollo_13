                  ...
                  int index, clock_t *elapsedTime)
{
  // load the image
  CvMat *raw_mat, *mat;
  mcvLoadImage2(filename, &raw_mat, &mat);

  // detect lanes
  vector<FLOAT> lineScores, splineScores;
  vector<Line> lanes;
  vector<Spline> splines;
  vector<Spline> splines_world;
  clock_t startTime = clock();
  mcvGetLanes(mat, raw_mat, &lanes, &lineScores, &splines, &splines_world, &splineScores,
